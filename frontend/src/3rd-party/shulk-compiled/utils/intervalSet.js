...
exports.intervalToArray = intervalToArray;
...
function complement(start, stop) {
  const result = new _antlr.default.IntervalSet();
  result.addInterval(new _antlr.default.Interval(start, stop + 1));
  ...
  if (toRemove.start === toRemove.stop - 1) {
    ...
    for (let n = 0; n < this.intervals.length; n++) {
      ...
      // that doesn't seem to be working when expected = [1,4] and toRemove = [1,3]  ------->
        ...
        this.intervals[pos] = new _antlr.default.Interval(existing.start, toRemove.start);
        const x = new _antlr.default.Interval(toRemove.stop, existing.stop);
      ...
      // check for upper boundary
        ...
        this.intervals[pos] = new _antlr.default.Interval(toRemove.stop, existing.stop);
      }
      pos += 1;
    }
  }
