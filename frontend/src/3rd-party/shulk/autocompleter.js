...
import AtomTransition from '../antlr4/transition/AtomTransition.js';
import RuleTransition from '../antlr4/transition/RuleTransition';
import SetTransition from '../antlr4/transition/SetTransition.js';
import RuleStopState from '../antlr4/state/RuleStopState.js';
import NotSetTransition from '../antlr4/transition/NotSetTransition.js';
...
import WildcardTransition from '../antlr4/transition/WildcardTransition.js';
...
function groupSuggestions(suggestions) {
  ...
  for (const s of suggestions) {
    ...
    if (register[s.token])
      register[s.token].ctxt = register[s.token].ctxt.concat(s.ctxt);
    else {
      grouped.push(s);
      register[s.token] = s;
    }
...
class Autocompleter {
  ...
  autocomplete(input) {
    ...
    if (initialState === undefined)
      throw new Error('Unexpected starting rule: ' + startingRule);
    ...
    for (const rule of parser.atn.ruleToStartState) {
      this.options.__cache[rule.ruleIndex] = this._executeATN(
        ...
        [
          [
            0,
            parser.atn.ruleToStartState[rule.ruleIndex],
            [],
    ...
    return this.options?.ignoreSuggestionsInNonDefaultChannels
      ? this._filterNonDefaultChannels(grouped, lexer)
      : grouped;
  }
  ...
  _executeATN(tokens, stack, options) {
    ...
    while (stack.length !== 0) {
      let [tokenStreamIndex, state, alreadyPassed, parserStack, recoveryData] =
        stack.pop();
      this.log(
        `[${state.stateNumber}] Next token: ${tokens[tokenStreamIndex]}. Already passed: ${alreadyPassed}`,
        stack
      );
      ...
      if (state instanceof RuleStopState && parserStack.length !== 0) {
        ...
        this.log(
          `[${state.stateNumber}] - Finished rule ${
            this.parserRuleNames[state.ruleIndex]
          } going to state ${nextState.toString()}`,
          stack
        );
      ...
      for (let i = state.transitions.length - 1; i >= 0; i--) {
        ...
        if (it.isEpsilon && !alreadyPassed.includes(it.target.stateNumber)) {
          if (
            it instanceof PrecedencePredicateTransition &&
            it.precedence < parserStack[parserStack.length - 1][2]
          )
            continue;
            ...
            if (nextToken === caret) {
              if (options.suggestRules.has(it.ruleIndex)) {
                suggestions.push(
                  new Suggestion(
                    it.ruleIndex,
                    parserStack.map((y) => y[0]),
                    true
                  )
                );
                continue;
              } else if (options.__cache) {
                options.__cache[it.target.ruleIndex]?.forEach((s) =>
                  suggestions.push(
                    new Suggestion(s.token, [
                      ...parserStack.map((x) => x[0]),
                      ...s.ctxt[0],
                    ])
                  )
                );
          ...
          const newParserStack =
            it instanceof RuleTransition
              ? [...parserStack, [it.ruleIndex, it.followState, it.precedence]]
              : parserStack;
          this.log(
            `[${state.stateNumber}] ${
              it instanceof RuleTransition
                ? `Entering rule ${it.ruleIndex}`
                : `Epsilon transition to ${it.target}`
            }`,
            stack
          );
          // Doesn't increase 'tokenStreamIndex' because it doesn't consume tokens
          if (limitNextState && it.target !== limitNextState) continue;
          let recoveryRules = options?.recovery
            ? options.recovery.filter((x) => x.ifInRule === it.ruleIndex)
            : [];
          if (it instanceof RuleTransition && recoveryRules.length > 0) {
            ...
            stack.push([
              ...
              alreadyPassed,
              parserStack,
              { suggestions: suggestions.length, recoveryRules },
            ]);
          }
          stack.push([
            tokenStreamIndex,
            ...
            it instanceof RuleTransition || state instanceof RuleStopState
              ? []
              : [it.target.stateNumber, ...alreadyPassed],
            newParserStack,
          ]);
          ...
          it instanceof AtomTransition ||
          it instanceof SetTransition
        ) {
          const nextToken = tokens[tokenStreamIndex];
          if (nextToken === caret) {
            this.log(
              `[${
                state.stateNumber
              }] In caret. Added suggestions: ${intervalToArray(
                it.label,
                parserStack.map((x) => x[0])
              ).map((x) => x.token)}`,
              stack
            ...
            this.log(
              `[${state.stateNumber}]Dead end. Expecting ${it.label} but found ${nextToken.type}`,
              stack
            );
          ...
          if (nextToken === caret) {
            suggestions.push(
              ...intervalToArray({
                intervals: [
                  {
                    start: antlr4.Token.MIN_USER_TOKEN_TYPE,
        ...
        } else if (alreadyPassed.includes(it.target.stateNumber))
          this.log('Dead end. Epsilon transition already passed', stack);
  ...
  _filterNonDefaultChannels(suggestions, lexer) {
    return suggestions.filter((x) => {
      ...
      return !actions.some((x) => {
        ...
        return (
          channel !== undefined && channel !== antlr4.Token.DEFAULT_CHANNEL
        );
      });
    });
  }
    ...
    for (let i = tokenStreamIndex + 1; i < tokens.length; i++) {
      if (tokens[i].type === andFindToken) {
        ...
        if (thenGoToRule) {
          stack.push([
            skipOne ? i + 1 : i,
            this.atn.ruleToStartState[thenGoToRule],
            [],
          ...
          stack.push([
            skipOne ? i + 1 : i,
            currentState.transitions[0].followState,
            [],
