...
export function complement(start, stop) {
  ...
  if (this.intervals !== null)
    this.intervals.forEach((toRemove) => removeRange.bind(result)(toRemove));
  return result;
}
  ...
  if (toRemove.start === toRemove.stop - 1) {
    ...
    for (let n = 0; n < this.intervals.length; n++) {
      ...
      // that doesn't seem to be working when expected = [1,4] and toRemove = [1,3]  ------->
        ...
        toRemove.start >= existing.start &&
        toRemove.stop <= existing.stop
      ) {
        this.intervals[pos] = new antlr4.Interval(
          existing.start,
          toRemove.start
        );
